...
- `prop_engine.py` — core logic: the math/model that produces prop projections and edges.
  - **Game model (`generate_game_picks`)** — the Daily Slate's ⭐ "model leans" on moneyline & total. Estimates each team's expected score from recent offense vs the opponent's recent defense (log5-style, scaled to league avg), projects the total, and derives a Pythagorean win probability, then compares to the market line. **Deliberately humble** (recent form only, no pitchers/injuries): it regresses team rates toward league average, anchors the moneyline to the market as a strong prior, and suppresses wild disagreements as "missing info." Don't "fix" it into overconfidence — that's a feature. Sport-parameterized in `GAME_MODEL_CONFIG`.
